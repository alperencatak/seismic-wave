...
(function () {
        ...
        kandilli: {
            ...
            parse: (data) => {
                ...
                return data.result.map(eq => ({
                    ...
                    depth:  eq.depth || 0,
                    lon:    eq.geojson?.coordinates?.[0] || 0,
                    lat:    eq.geojson?.coordinates?.[1] || 0,
                    tsunami: false,
                    url:    null
                }));
            }
    ...
    async function fetchSingleSource(key) {
        ...
        if (state.timeframe === 'custom' && state.customStart && state.customEnd) {
            if (key === 'usgs') {
                ...
                url = `https://api.orhanaydogdu.com.tr/deprem/kandilli/archive?date=${state.customStart}&date_end=${state.customEnd}&limit=500`;
            } else if (key === 'emsc') {
    ...
    function applyFilters() {
        ...
        // 1. Time filter for custom timeframe
        if (state.timeframe === 'custom' && state.customStart && state.customEnd) {
