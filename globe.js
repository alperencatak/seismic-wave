...
class SeismicGlobe {
    ...
    setupInteractions() {
        ...
        d3.select(this.canvas).on('click', function(event) {
            ...
            if (self.isDragging && 
                Math.abs(event.x - self.dragStart[0]) > 5 && 
                Math.abs(event.y - self.dragStart[1]) > 5) return;
                
            if (!self.countries) return;
            ...
            if (clickedCountry) {
                ...
                d3.select('#btn-rotate').classed('active', false);
            } else {
                self.selectCountry(null);
                self.autoRotate = true;
                d3.select('#btn-rotate').classed('active', true);
            }
        });
    }
    ...
    setHovered(feature) {
        ...
        if (feature) {
            ...
            tooltip.textContent = feature.properties.name || "Unknown Land";
    ...
    /**
     * Highlights a specific earthquake on the globe
     */
    setSelectedQuake(id) {
        this.selectedQuakeId = id;
    }
    ...
    smoothRotateTo(target) {
        let currentRotation = this.projection.rotate();
        
        // Normalize accumulated rotation longitude to be within [-180, 180]
        let currentLon = currentRotation[0] % 360;
        if (currentLon > 180) currentLon -= 360;
        else if (currentLon < -180) currentLon += 360;
        
        let targetLon = -target[0];
        
        // Find shortest angular path
        if (targetLon - currentLon > 180) currentLon += 360;
        else if (currentLon - targetLon > 180) targetLon += 360;

        const normalizedCurrent = [currentLon, currentRotation[1], currentRotation[2] || 0];
        const targetRotation = [targetLon, -target[1], currentRotation[2] || 0];

        // Interpolate rotation transitions using D3 transition timer
        const interpolator = d3.interpolate(normalizedCurrent, targetRotation);
        
        d3.select(this.canvas).interrupt();

        this.transition = d3.select(this.canvas).transition()
            .duration(1200)
    ...
    drawEarthquakes() {
        ...
        this.earthquakes.forEach(eq => {
            ...
            if (this.isPointVisible(coords)) {
                ...
                const isSelected = (eq.id === this.selectedQuakeId);
                ...
                if (isSelected) {
                    size *= 3.5;
                    glowColor = 'rgba(255, 255, 255, 0.3)'; // Subtle white glow
                    // Keep original fillStyle to match magnitude palette
                }

                // Draw glowing aura
                this.ctx.beginPath();
                this.ctx.arc(projected[0], projected[1], size * (isSelected ? 1.5 : 2.2), 0, Math.PI * 2);
                ...
                this.ctx.lineWidth = isSelected ? 2 : 0.8;
                this.ctx.fill();
                this.ctx.stroke();

                if (isSelected) {
                    this.ctx.fillStyle = fillStyle; // Match magnitude color
                    this.ctx.font = 'bold 20px "Inter", sans-serif';
                    this.ctx.textAlign = 'center';
                    this.ctx.shadowColor = 'rgba(0,0,0,0.9)';
                    this.ctx.shadowBlur = 6;
                    this.ctx.fillText(`M ${mag.toFixed(1)}`, projected[0], projected[1] - size - 12);
                    this.ctx.shadowBlur = 0; // reset
                }
            }
        });
